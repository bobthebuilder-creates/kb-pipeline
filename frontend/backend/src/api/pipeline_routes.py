...
def _run_pipeline_job(job_id: str, input_path: str, indexing_method: str) -> None:
    ...
    try:
        # Prepare LLM config (client optional for now)
        job.stage = "llm_init"
        job.message = "Initializing LLM configuration"
        _PIPELINE_JOBS[job_id] = job

        llm_config = default_llm_config()

        # Try to create a client, but don't fail the pipeline if it doesn't work.
        try:
            _ = create_llm_client(llm_config)
            logger.info("[PIPELINE] LLM client initialized successfully.")
        except Exception as e:
            logger.warning(
                f"[PIPELINE] LLM client initialization failed (non-fatal for now): {e}"
            )
